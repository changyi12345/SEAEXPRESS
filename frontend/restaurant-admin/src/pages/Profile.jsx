...
export default function Profile() {
  ...
  const fetchRestaurant = async () => {
    try {
      ...
      setPromotions(restaurantData.promotions || [])
      setClosingDays(restaurantData.closingDays || [])
      setFormData({
        name: restaurantData.name || '',
        nameMyanmar: restaurantData.nameMyanmar || '',
        phone: restaurantData.phone || '',
        email: restaurantData.email || '',
        profileImage: restaurantData.profileImage || null,
        ...
        paymentAccounts: restaurantData.paymentAccounts || {
          kbzpay: {
            accountName: '',
            accountNumber: '',
            phone: ''
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage('')
    try {
      await axios.put('/restaurant-owners/my-restaurant', formData)
  ...
  const handleAddPromotion = async (e) => {
    ...
    setMessage('')
    try {
      const res = await axios.post('/restaurant-owners/my-restaurant/promotions', promotionForm)
      setPromotions(res.data.restaurant.promotions)
      setPromotionForm({
        title: '',
        titleMyanmar: '',
        description: '',
  ...
  const confirmDeletePromotion = async () => {
    setLoading(true)
    try {
      const res = await axios.delete(`/restaurant-owners/my-restaurant/promotions/${confirmModal.id}`)
      setPromotions(res.data.restaurant.promotions)
  ...
  const handleAddClosingDay = async (e) => {
    ...
    setMessage('')
    try {
      const res = await axios.post('/restaurant-owners/my-restaurant/closing-days', closingDayForm)
      setClosingDays(res.data.restaurant.closingDays)
  ...
  const confirmDeleteClosingDay = async () => {
    setLoading(true)
    try {
      const res = await axios.delete(`/restaurant-owners/my-restaurant/closing-days/${confirmModal.id}`)
      setClosingDays(res.data.restaurant.closingDays)
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-6">
        ...
        <div className="border-b pb-6 mb-6">
          <h2 className="text-xl font-bold mb-4">📷 Restaurant Profile Image</h2>
          <ImageUpload
            label="Upload or Change Restaurant Profile Image"
            existingImage={formData.profileImage}
            onUpload={(imageUrl) => setFormData({ ...formData, profileImage: imageUrl })}
          />
          <p className="text-sm text-gray-600 mt-2">This image will be displayed to customers when they browse restaurants</p>
        </div>

        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <label className="block mb-2 font-semibold">Restaurant Name (English)</label>
            <input
              type="text"
              value={formData.name}
          ...
          <div>
            <label className="block mb-2 font-semibold">Restaurant Name (Myanmar)</label>
            <input
              type="text"
        ...
        <div>
          <p className="mb-2 font-semibold">Status: {restaurant.isApproved ? 'Approved' : 'Pending Approval'}</p>
        </div>
